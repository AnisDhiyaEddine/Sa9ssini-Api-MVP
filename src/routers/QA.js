...
router.get("/answers/:questionID/bestAnswer", auth, async (req, res) => {
  ...
  try {
    let max = 0;
    let question = await Question.findById(_id).cache({ key: req.user._id });
    if (!question) {
      res.status(404).send({ error: "not found" });
    }
    let maxVotes = 0;
    ...
    question.answers.forEach(async (el) => {
      let answer = await Answer.findById(el.answer).cache({
        key: req.user._id,
      });
...
router.get("/answers/:questionID", auth, async (req, res) => {
  ...
  try {
    ...
    let answers = await Answer.find({ question: _id }).cache({
      key: req.user._id,
    });
    res.send(answers);
  } catch (error) {
...
//vote an answer
router.patch("/answers/:answerID/vote", auth, clearCache, async (req, res) => {
  const _id = req.params.answerID;
  try {
    const answer = await Answer.findOne({ _id }).cache({ key: req.user._id });
    if (!answer) {
      return res.status(404).send();
    }
